...
pub async fn execute(args: WhyArgs, inverted: bool) -> Result<i32> {
    ...
    let installed_repo = Arc::new(phpx_pm::repository::InstalledRepository::new(vendor_dir));
    installed_repo.load().await.ok();
    let mut installed_packages = installed_repo.get_packages().await;

    let root_package = phpx_pm::Package {
    ...
    let constraint = if constraint_str != "*" {
        let parser = phpx_semver::VersionParser;
...
fn print_tree(results: &[DependencyResult], root: &Arc<phpx_pm::Package>) {
    println!(
        "{} {}",
        root.name,
